...
class ModelManager:
    ...
    def load_or_download_model(self, stage: str, model_name: str,
        ...
        # IMPORTANT: When pretrained=False, we should ALWAYS create a fresh model with random weights
        if not pretrained:
            self.logger.info(f"Creating model with random initialization (pretrained=False)")
            model = timm.create_model(
                model_name,
                pretrained=False,  # This ensures random weights
                num_classes=num_classes
            )
            ...
            self.logger.info(f"Model created with random weights: {model_name}")
            self.logger.info(f"Total parameters: {total_params:,}")
            self.logger.info(f"Trainable parameters: {trainable_params:,}")

            return model

        # If pretrained=True, then try to load from cache or download
        # Try to load from local cache first
        if self.check_local_model_exists(model_dir):
            self.logger.info(f"Loading pretrained model from local directory: {model_dir}")
            model = self.load_local_model(model_name, model_dir, target_num_classes=num_classes)

            if model is not None:
                model = model.to(device)
                self.logger.info("Successfully loaded pretrained model from local cache")
                return model
            else:
        ...
        # Download and save model (only if pretrained=True)
        if self.download_and_save_model(model_name, model_dir, pretrained=True, num_classes=0):
        ...
        if model is None:
            ...
            model = timm.create_model(
                model_name,
                pretrained=True,  # We only reach here if pretrained=True
                num_classes=num_classes
            )
        ...
        total_params = sum(p.numel() for p in model.parameters())
        trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        self.logger.info(f"Pretrained model loaded: {model_name}")
        self.logger.info(f"Total parameters: {total_params:,}")
        self.logger.info(f"Trainable parameters: {trainable_params:,}")
