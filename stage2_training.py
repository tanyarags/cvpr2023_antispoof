...
class MaxViTWithFeatures(nn.Module):
    ...
    def initialize_with_backbone(self, backbone_model):
        ...
        if hasattr(backbone_model, 'head'):
            self.backbone = nn.Sequential(*list(backbone_model.children())[:-1])
        elif hasattr(backbone_model, 'classifier'):
            self.backbone = nn.Sequential(*list(backbone_model.children())[:-1])
        else:
            # For models where we can't easily remove the head, create a feature extractor
        ...
        with torch.no_grad():
            ...
            if hasattr(self.backbone, 'forward_features'):
                ...
                # Try standard forward pass and handle the output
                try:
                    backbone_output = self.backbone(dummy_input)
                    if isinstance(backbone_output, tuple):
                        backbone_output = backbone_output[0]
                except:
                    # Fallback - assume the original model structure
                    backbone_output = torch.randn(1, 1024)  # Common feature dim
...
class Stage2Trainer:
    ...
    def create_model(self):
        ...
        backbone_model = self.model_manager.load_or_download_model(
            ...
            num_classes=1000,  # Keep original classes for feature extraction
